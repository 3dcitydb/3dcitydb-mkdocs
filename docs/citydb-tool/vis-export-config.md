...
above can be set here, plus the 3D Tiles-specific `"implicitGeometryInstancing"`, `"enableOutline"`, and
`"outlineMergeCoplanar"` properties.
...
| [`"outlineMergeCoplanar"`](vis-export-3dtiles.md#rendering-surface-outlines)                                                 | Maximum dihedral angle in degrees (`0` ≤ angle < `90`) up to which edges shared by two coplanar surfaces of the same feature and surface type are exported without outlines; `0` merges only exactly coplanar surfaces. Requires `"enableOutline"`. | none (off)    |
